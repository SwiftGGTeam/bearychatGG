...
def get_html_data():
    f = urllib.request.urlopen("http://swiftandpainless.com/feed/")
...
def check():
    old_data = read_old_data("swiftandpainless_profile")
    html = get_html_data()
    new_data = cal_new_data(old_data, get_full_data(html, "item"))
    if new_data:
        send_data = {"text": concat_data("Swift and Painless", new_data), "markdown": True}
        ...
        write_new_data("swiftandpainless_profile", new_data)
