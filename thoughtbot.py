...
def get_html_data():
    f = urllib.request.urlopen("https://robots.thoughtbot.com/tags/swift")
    soup = BeautifulSoup(f.read(), "html.parser")  # .decode('gbk', 'ignore').encode('utf-8', 'ignore')
    return soup

def get_full_data(html, title):
    titles = html.find_all("h2", "mini-post-title")
    full_data = []
    for title in titles:
        full_data.append(title.a["href"] + "GGBREAK" + title.a.string)
    return full_data
...
def check():
    old_data = read_old_data("thoughtbot_profile")
    html = get_html_data()
    new_data = []
    for title in anchor_titles:
        new_data += cal_new_data(old_data, get_full_data(html, title))
    if new_data:
        send_data = {"text": concat_data("thoughtbot", new_data), "markdown": True}
        ...
        write_new_data("thoughtbot_profile", new_data)
