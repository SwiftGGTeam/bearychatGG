...
def get_html_data():
    f = urllib.request.urlopen("https://www.mikeash.com/pyblog/rss.py")
    soup = ET.fromstring(f.read()) # .decode('gbk', 'ignore').encode('utf-8', 'ignore')
    return soup

def get_full_data(html, tag):
    full_data = []
    items = get_html_data()[0].findall(".//item")
    for item in items:
...
def check():
    old_data = read_old_data("nsblog_profile")
    html = get_html_data()
    new_data = cal_new_data(old_data, get_full_data(html, "item"))
    if new_data:
        send_data = {"text": concat_data("NSBlog(mikeash.com)", new_data), "markdown": True}
        ...
        write_new_data("nsblog_profile", new_data)
